...
public class TriangleElementRegistry extends AbstractImageElementRegistry {
  ...
  protected BufferedImage buildImage(IAvatarInfo avatarInfo) {
    int size =
        Math.min(
            avatarInfo.getWidth() - (avatarInfo.getMargin() + avatarInfo.getPadding()) * 2,
    ...
    while (p > 0) {
      ...
      switch (i % 4) {
        ...
        case 1:
          polygon =
              new Polygon(new int[] {size - 1, size - 1, size - p - 1}, new int[] {0, p - 1, 0}, 3);
          break;
        case 2:
          polygon =
              new Polygon(
                  new int[] {size - 1, size - p - 1, size - 1},
                  new int[] {size - 1, size - 1, size - p - 1},
                  3);
          break;
        case 3:
          polygon =
              new Polygon(new int[] {0, 0, p - 1}, new int[] {size - 1, size - p - 1, size - 1}, 3);
          break;
      }
