 ...
 * <p>
 *
 * <p>Current implementation uses only the lower 32 bits of identicon code.
 *
 * @author don
 */
public class NineBlockIdenticonRenderer {
  ...
  private static final byte[] patchTypes[] = {
    patch0, patch1, patch2, patch3, patch4, patch5, patch6, patch7, patch8, patch9, patch10,
  ...
  /**
   * Returns the size in pixels at which each patch will be rendered before they are scaled down to
   * requested identicon size.
   *
   * @return
   */
  ...
  /**
   * Set the size in pixels at which each patch will be rendered before they are scaled down to
   * requested identicon size. Default size is 20 pixels which means, for 9-block identicon, a 60x60
   * image will be rendered and scaled down.
   *
   ...
   * <p>
   *
   * <p>Size of the returned identicon image is determined by patchSize set using {@link
   * setPatchSize}. Since a 9-block identicon consists of 3x3 patches, width and height will be 3
   * times the patch size.
   *
  ...
  protected BufferedImage renderQuilt(int code, int size) {
    ...
    // middle patch
    drawPatch(
        g, blockSize, blockSize, blockSize, middleType, 0, middleInvert, fillColor, strokeColor);
