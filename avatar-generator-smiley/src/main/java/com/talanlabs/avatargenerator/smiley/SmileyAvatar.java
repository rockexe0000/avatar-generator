...
public class SmileyAvatar {
  ...
  private static URLElementRegistry newAllElementRegistry() {
    ...
    for (SmileyElementType smileyElementType : SmileyElementType.values()) {
      elementRegistry.putElement(
          smileyElementType.name(),
          URLElementRegistry.lsPngURLs(
              SmileyAvatar.class.getClassLoader(), smileyElementType.path));
    }
  ...
  public static ElementRegistry newDefaultElementRegistry() {
    ...
    elementRegistry.putGroup(
        ElementInfo.of(SmileyElementType.shape.name()),
    ...
    elementRegistry.putGroup(
        ElementInfo.of(SmileyElementType.moreShape.name()),
        ElementInfo.of(SmileyElementType.eyeBig.name()),
        ElementInfo.of(SmileyElementType.happyMouth.name()));
    elementRegistry.putGroup(
        ElementInfo.of(SmileyElementType.moreShape.name()),
    ...
    elementRegistry.putGroup(
        ElementInfo.of(SmileyElementType.shape.name()),
        ElementInfo.of(SmileyElementType.eye.name()),
  ...
  public static Avatar.AvatarBuilder newAccessoriesAvatarBuilder() {
    return Avatar.newBuilder()
        .size(98, 98)
        .elementRegistry(newAccessoriesElementRegistry())
        .colorizeFunction(new RandomColorizeFunction())
        .padding(8)
        .layers(
            new ShadowLayer(),
            new ScoreShadowLayer(),
  ...
  public static Avatar.AvatarBuilder newEyeMouthAvatarBuilder() {
    return Avatar.newBuilder()
        .size(82, 82)
        .elementRegistry(newEyeMouthElementRegistry())
  ...
  public static ElementRegistry newEyeMouthElementRegistry() {
    URLElementRegistry elementRegistry = newAllElementRegistry();
    elementRegistry.putGroup(
        ElementInfo.of(SmileyElementType.eyeBig.name(), 0, -10),
  ...
  public static Avatar.AvatarBuilder newGhostAvatarBuilder() {
    return Avatar.newBuilder()
        .size(82, 82)
        .elementRegistry(newGhostElementRegistry())
        .colorizeFunction(
            (avatarInfo, element) -> {
              if (SmileyElementType.moreShape.name().equals(element)) {
                Color color =
                    AvatarUtils.defaultColors.get(
                        (int) (avatarInfo.getCode() % AvatarUtils.defaultColors.size()));
  ...
  public static ElementRegistry newGhostElementRegistry() {
    URLElementRegistry elementRegistry = newAllElementRegistry();
    elementRegistry.putElement(
        SmileyElementType.moreShape.name(),
        Collections.singletonList(
            SmileyAvatar.class.getResource(
                "/com/talanlabs/avatargenerator/smiley/images/MoreShape/30-2.png")));
